...
const PlacesList: React.FC<Props> = ({ places }) => {
  ...
  // Group places by day for multi-day planning
  ...
  const isMultiDay = Object.keys(placesByDay).length > 1;
  ...
  const renderPlacesList = (dayPlaces: typeof places, dayNumber?: number) => (
    <ul className="space-y-4">
      {dayPlaces.map((place, i) => {
        const totalTimeAtPlace = (place.walkingTime || 0) + (place.visitDuration || 0);
        
        return (
          <li key={`${dayNumber || 1}-${i}`} className="bg-white rounded-lg shadow p-4 border border-gray-100 flex flex-col gap-3">
            {place.photoUrl && (
      ...
      {isMultiDay ? (
        Object.keys(placesByDay)
          ...
          .map(dayKey => {
            ...
            const dayPlaces = placesByDay[dayNumber];
            const dayWalkingTime = dayPlaces.reduce((sum, place) => sum + (place.walkingTime || 0), 0);
            const dayVisitTime = dayPlaces.reduce((sum, place) => sum + (place.visitDuration || 0), 0);
            
            return (
              <div key={dayKey} className="space-y-3">
                <div className="bg-primary/10 rounded-lg p-3 border border-primary/20">
                  <div className="flex justify-between items-center">
                    <h3 className="text-lg font-semibold text-primary">Day {dayNumber}</h3>
                    <div className="text-sm text-gray-600">
                      {dayPlaces.length} places • {dayWalkingTime + dayVisitTime} min total
                    </div>
                  </div>
                </div>
                {renderPlacesList(dayPlaces, dayNumber)}
              </div>
            );
          })
      ) : (
